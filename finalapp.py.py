...
def load_data(filepath):
    ...
    try:
        ...
        # IMPORTANT: Keep rows even if some features are NaN, only drop if targets/essentials are NaN
...
def generate_future_features(historical_data, geo, start_year, num_years, features_to_forecast):
    ...
    # Aggregate historical data by year (mean) to get a single value per year for trend calculation
    geo_data_agg = geo_data.groupby('Year', as_index=False)[features_to_forecast].mean()
    # Ensure 'Year' is int after grouping if it changes type
    geo_data_agg['Year'] = geo_data_agg['Year'].astype(int)

    # Use the last row of the *aggregated* data for the last known values
    last_known_values = geo_data_agg[geo_data_agg['Year'] == last_known_year].iloc[-1]

    future_df_list = []

    # --- Extrapolation Method (Simple Linear Trend per feature based on aggregated data) ---
    growth_rates = {}
    for feature in features_to_forecast:
        if feature != 'Year':
            feature_series = geo_data_agg.set_index('Year')[feature].dropna() # Use aggregated series
    ...
    for i in range(num_years):
        ...
        for feature in features_to_forecast:
            if feature != 'Year':
                base_value = last_known_values.get(feature, np.nan)

                if pd.isna(base_value):
                     # Fallback to overall mean for the feature in the *aggregated* geo data
                     geo_feature_mean = geo_data_agg[feature].mean()
                ...
                if feature == 'Actual Population' and future_row[feature] < 0:
                    future_row[feature] = 0 # Or a more reasonable floor
...
if not available_geos:
    ...
    st.stop()
if 'geo_filter' not in st.session_state or st.session_state.geo_filter not in available_geos:
     st.session_state.geo_filter = available_geos[0] # Default or reset if invalid

geo_filter = st.sidebar.selectbox("Province/Territory:", available_geos, index=available_geos.index(st.session_state.geo_filter))
st.session_state.geo_filter = geo_filter
...
for food in food_cols:
    adjustments[food] = st.sidebar.slider(f"{food}", -10, 10, 0, key=f"slider_{food}_{geo_filter}")
...
if baseline_future_features.empty or baseline_future_features[selected_features].isnull().all().all():
     st.markdown("---")
     st.warning(f"Could not generate a valid baseline forecast for {geo_filter}, possibly due to insufficient historical data. Cannot proceed.")
     st.stop()
...
for food, percent_change in adjustments.items():
    if food in adjusted_future_features.columns:
        if pd.api.types.is_numeric_dtype(adjusted_future_features[food]):
            ...
            # adjusted_future_features.loc[:, food] = adjusted_future_features[food].clip(lower=0) # Optional clip
        else:
...
try:
    ...
    # Impute NaNs before scaling (using fillna(0) as a simple strategy)
    ...
    st.error(f"Feature mismatch error during scaling: {e}. Columns in generated data might not match expected features ('{selected_features}'). Check 'selected_features.pkl'.")
    st.stop()
except ValueError as e:
    st.error(f"Scaling error: {e}. Check for NaNs or unexpected data types before scaling. Use the debug checkbox.")
    st.stop()
except Exception as e:
...
results_df = pd.DataFrame({
    ...
    'Baseline Diabetes': baseline_pred_d.clip(min=0),
    'Adjusted Diabetes': adjusted_pred_d.clip(min=0),
    'Baseline HBP': baseline_pred_hbp.clip(min=0),
    'Adjusted HBP': adjusted_pred_hbp.clip(min=0)
})
# You can uncomment the rounding if you prefer cleaner tables
...
fig_d.add_trace(go.Scatter(
    x=results_df['Year'], y=results_df['Adjusted Diabetes'],
...
fig_hbp.add_trace(go.Scatter(
    x=results_df['Year'], y=results_df['Adjusted HBP'],
